...
impl<C> EndpointsBuilder<C> {
    ...
    pub const fn new(base_transport: Http<C>) -> Self {
        Self { base_transport, endpoints: Endpoints(vec![]) }
    }
}
    ...
    MevHttp<C>: Transport,
{
    ...
    pub fn endpoint_with_bundle_signer(mut self, url: Url, bundle_signer: BundleSigner) -> Self {
        self.endpoints.add(MevHttp::new(url, self.base_transport.clone(), bundle_signer).boxed());
    ...
    pub fn endpoint(mut self, url: Url) -> Self {
        self.endpoints.add(Http::with_client(reqwest::Client::new(), url).boxed());
    ...
    pub fn beaverbuild(mut self) -> Self {
        self.endpoints.add(Http::with_client(reqwest::Client::new(), "https://rpc.beaverbuild.org".parse().unwrap()).boxed());
    ...
    pub fn titan(mut self, bundle_signer: BundleSigner) -> Self {
        self.endpoints
            .add(MevHttp::new("https://rpc.titanbuilder.xyz".parse().unwrap(), self.base_transport.clone(), bundle_signer).boxed());
    ...
    pub fn rsync(mut self) -> Self {
        self.endpoints.add(Http::with_client(reqwest::Client::new(), "https://rsync-builder.xyz/".parse().unwrap()).boxed());
    ...
    pub fn flashbots(mut self, bundle_signer: BundleSigner) -> Self {
        self.endpoints
            .add(MevHttp::new("https://relay.flashbots.net".parse().unwrap(), self.base_transport.clone(), bundle_signer).boxed());

        self
    }

    /// Adds Gmbit.
    pub fn gmbit(mut self) -> Self {
        self.endpoints.add(Http::with_client(reqwest::Client::new(), "https://builder.gmbit.co/rpc".parse().unwrap()).boxed());

        self
    }

    /// Adds Lokibuilder.
    pub fn lokibuilder(mut self) -> Self {
        self.endpoints.add(Http::with_client(reqwest::Client::new(), "https://rpc.lokibuilder.xyz".parse().unwrap()).boxed());
        self
    }
