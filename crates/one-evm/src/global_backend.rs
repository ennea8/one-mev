...
impl GlobalBackend {
    ...
    fn on_request(&mut self, req: BackendFetchRequest) {
        debug!("🔰🌐[[one-evm] on_request: {:?}", req);
        match req {
            BackendFetchRequest::Basic(addr, sender) => {
                let acc = self.db.accounts.get(&addr);

                debug!("🔰🌐[one-evm] on_request BackendFetchRequest::Basic cur acc: {:?}", acc);
            ...
            BackendFetchRequest::Storage(addr, idx, sender) => {
                ...
                debug!("🔰🌐[one-evm] on_request BackendFetchRequest::Storage cur value: {:?}", value);
            ...
            BackendFetchRequest::BlockHash(number, sender) => {
                ...
                debug!("🔰🌐[one-evm] on_request BackendFetchRequest::BlockHash cur hash: {:?}", hash);
